  ...
  render() {
    ...
    return html`
      <div class="form" style="--editor-width: ${editorWidth}">
        ...
        <div class="form-group">
          <label class="color-input-wrapper">燃气费数据颜色：
            <input 
              type="color" 
              @change=${this._valueChanged}
        ...
        <div class="balance-entity-section">
          ...
          ${this.config.entities && this.config.entities.length > 0 ? html`
            <div class="selected-balance-entities">
              <div class="selected-balance-label">已选择的燃气实体：</div>
...
customElements.define('gas-balance-card-editor', GasBalanceCardEditor);

class GasBalanceCard extends LitElement {
  static getConfigElement() {
    return document.createElement("gas-balance-card-editor");
  }
  ...
  get _processedDayData() {
    ...
    const daylistRaw = selectedEntity.attributes.daylist;
    const daylist = daylistRaw.slice(Math.max(daylistRaw.length - 30, 0));
    const currentDay = daylist[daylist.length - 1] || {};
    return {
      electricity: daylist.map(item => ({
        x: new Date(item.day.split(' ')[0]).getTime(),
        y: Number(item.dayGasNum) || 0
      })),
      cost: daylist.map(item => ({
        x: new Date(item.day.split(' ')[0]).getTime(),
        y: Number(item.dayGasCost) || 0
      })),
      current: {
        ele: currentDay.dayGasNum || 0,
        cost: currentDay.dayGasCost || 0,
        days: daylist.length
      }
    };
  }
  ...
  get _processedMonthData() {    
    ...
    const lastmonthlist = [...lastYearBills ].slice(0, 12);
    const monthlist = [...thisYearBills].slice(0, 12);
    const lastmonthlistDay = lastmonthlist[lastmonthlist.length - 1];
    const monthlistDay = monthlist[monthlist.length - 1];
    return {
      electricity: monthlist.map(item => ({
        x: new Date(item.month.substr(0,7)+'-01').getTime(),
        y: Number(item.monthGasNum) || 0
      })),
      cost: monthlist.map(item => ({
        x: new Date(item.month.substr(0,7)+'-01').getTime(),
        y: Number(item.monthGasCost) || 0
      })),
      current: {
        ele: monthlistDay?.monthGasNum || 0,
        cost: monthlistDay?.monthGasCost || 0,
      ...
      lastcurrent: {
        ele: lastmonthlistDay?.monthGasNum || 0,
        cost: lastmonthlistDay?.monthGasCost || 0,
    ...
    return {
        ...
        points: [
          {
            ...
            label: {
              ...
              text: `${maxElectricity.toFixed(2)}m³`
            }
          },
          {
            x: maxElectricityPoint.x,
      ...
      tooltip: {
        ...
        custom: function({ series, seriesIndex, dataPointIndex, w }) {
          ...
          const seriesInfo = [
            { name: '日气量', unit: 'm³', color: this.colorNum },
            { name: '日气费', unit: '元', color: this.colorCost }
          ];
          let tooltipHTML = `
    ...
    return {
      ...
      tooltip: {
        ...
        custom: function({ series, seriesIndex, dataPointIndex, w }) {
          ...
          const displayDate = `${currentDate.getFullYear()}-${String(currentDate.getMonth() + 1).padStart(2, '0')}`;
          const seriesInfo = [
            { name: '上年气量', unit: 'm³', color: "#f85000" },
            { name: '本年气量', unit: 'm³', color: this.colorNum },
            { name: '上年气费', unit: '元', color: "#f30660" },
            { name: '本年气费', unit: '元', color: this.colorCost }
          ];
          let tooltipHTML = `
            <div style="background: ${BgColor};color: ${Color};padding: 8px;border-radius: 4px;border: 1px solid ${Color};">
              <div style="font-weight: bold; font-size: 12px;color: ${Color};  border-bottom: 1px dashed #999;">
 ...
 /*渲染日度用气条形图*/
  renderDayBar(usage) {
    ...
    // Gas usually doesn't have tip/peak/valley breakdown in this sensor, so we just use total
    const total = usage.dayGasNum || 0;
    if (total === 0) return '';
    
    // Just use 'normal' color for single bar
    ...
    const texts = [];
    if (total > 0) {
      texts.push(html`<div class="usage-bar-text normal" style="color: ${Color}; text-shadow: ${Shadow}; width: 100%; left: 0;">${total}</div>`);
    }
    
    const labels = [];
    if (total > 0) labels.push(html`<div class="usage-label normal" style="color: ${Color}; text-shadow: ${Shadow}; width: 100%; left: 0;">用量</div>`);
  ...
  renderUsageBar(usage) {
    ...
    const total = usage.monthGasNum || 0;
    if (total === 0) return '';
  ...
  renderYearUsageBar(usage) {
    ...
    const Color = theme === 'on' ? 'rgb(0, 0, 0)' : 'rgb(255, 255, 255)';
    const Shadow = theme === 'on' ? '0 1px 2px rgba(255, 255, 255, 0.3)' : '0 1px 2px rgba(50, 50, 50, 0.6)';
    const total = usage.yearGasNum || 0;
    if (total === 0) return '';
  ...
  /*渲染价格区块*/
  renderPriceBlock(price) {
     return html`<div class="price-item-block">单价：${price}</div>`;
  }
  ...
  getMinMaxUsageDays() {
    ...
    const validDays = monthDays.filter(day => day.dayGasNum !== undefined && day.dayGasNum !== null);
    if (validDays.length === 0) return { minDays: [], maxDays: [] };
    const minUsage = Math.min(...validDays.map(day => parseFloat(day.dayGasNum)));
    const maxUsage = Math.max(...validDays.map(day => parseFloat(day.dayGasNum)));
    const minDays = validDays
        .filter(day => parseFloat(day.dayGasNum) === minUsage)
        .map(day => parseInt(day.day.split('-')[2], 10).toString());
    const maxDays = validDays
        .filter(day => parseFloat(day.dayGasNum) === maxUsage)
        .map(day => parseInt(day.day.split('-')[2], 10).toString());
  ...
  renderCalendar() {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      const dayContent = html`
        <div>${i}</div>
        ${dayData ? html`
          <div class="electricity-num" style="color: ${this.colorNum}">${dayData.dayGasNum}m³</div>
          <div class="electricity-cost" style="color: ${this.colorCost}">${dayData.dayGasCost}元</div>
        ` : ''}
      `;
    ...
    const bottomRow = html`
      ...
      <div class="cell summary-info" style="grid-area: id99;">
        ${this.monthData ? html`
          <div><span  style="color: ${this.colorNum}">月气量: ${this.monthData.monthGasNum}m³</span></div>
          <div><span  style="color: ${this.colorCost}">月气费: ${this.monthData.monthGasCost}元</span></div>
        ` : html`<div></div>`}
      </div>
    `;
    return html`
  ...
  renderMain() {
    ...
    const Shadow = theme === 'on' ? '0 1px 2px rgba(255, 255, 255, 0.3)' : '0 1px 2px rgba(50, 50, 50, 0.6)';
    const svgpath = theme === 'on' ? '/gas_balance_mqtt-local/icon/gas-on.svg' : '/gas_balance_mqtt-local/icon/gas-off.svg';
    ...
    // Gas doesn't typically have prepaid/postpaid flag in this sensor, so default to standard display
    const isprepaid = "否";
    
    // 渲染阶梯区域内容
    let ladderContent = '';
    
    // Adapt ladder logic to read from flat attributes of gas_balance_mqtt
    const attrs = selectedEntity.attributes;
    if (attrs && attrs.yearly_step_accumulated_usage !== undefined) {
        const totalUsage = attrs.yearly_step_accumulated_usage;
        const secondLevelStart = attrs.yearly_step_2_start_volume || 400;
        const thirdLevelStart = attrs.yearly_step_3_start_volume || 1680;
        ...
        const minArrowPosition = 1;
        const maxArrowPosition = 99;
        const constrainedArrowPosition = Math.max(minArrowPosition, Math.min(maxArrowPosition, bubblePosition));
        
        // Prices
        const p1 = attrs.year_step_1_price || 0;
        const p2 = attrs.year_step_2_price || 0;
        const p3 = attrs.year_step_3_price || 0;
    ...
    let priceContent = '';
      ...
      const currentDayUsage = daylist.length > 0 ? daylist[daylist.length - 1] : null;
      ...
      const yearUsage = this.getYearUsage(selectedEntity.attributes?.yearlist || [], currentYear); // Sensor might not have yearlist yet
        ...
        <div class="usage-grid">
          <div class="usage-section" style="background: ${BgColor2};">
            ...
            ${currentDayUsage ? html`
              <div class="usage-amount">
                <span class="usage-electricity">${currentDayUsage.dayGasNum}m³</span>
                <span class="usage-cost">${currentDayUsage.dayGasCost}元</span>
              </div>
          ...
          <div class="usage-section" style="background: ${BgColor2}">
            ...
            ${currentMonthUsage ? html`
              <div class="usage-amount">
                <span class="usage-electricity">${currentMonthUsage.monthGasNum}m³</span>
                <span class="usage-cost">${currentMonthUsage.monthGasCost}元</span>
              </div>
          ...
          <div class="usage-section" style="background: ${BgColor2}">
            ...
            ${previousMonthUsage ? html`
              <div class="usage-amount">
                <span class="usage-electricity">${previousMonthUsage.monthGasNum}m³</span>
                <span class="usage-cost">${previousMonthUsage.monthGasCost}元</span>
              </div>
          ...
          <div class="usage-section" style="background: ${BgColor2}">
            ...
            ${yearUsage ? html`
              <div class="usage-amount">
                <span class="usage-electricity">${yearUsage.yearGasNum}m³</span>
                <span class="usage-cost">${yearUsage.yearGasCost}元</span>
              </div>
          ...
          <div class="top-section">
            ...
            <div class="balance-section">
              <div class="top-content">
                ...
                <div class="balance-time">${selectedEntity.attributes?.last_record_time || selectedEntity.attributes?.date || ''}</div>
...
customElements.define('gas-balance-card', GasBalanceCard);

window.customCards = window.customCards || [];
window.customCards.push(
  {
    type: 'gas-balance-card',
    name: '燃气信息卡片',
    description: '显示燃气信息，包括余额、单价、用气阶梯和用气统计'
  }
);
